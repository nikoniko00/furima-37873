  ...
  describe 'ユーザー新規登録' do
    ...
    context '新規登録できないとき' do
      it 'nicknameが空では登録できない' do
        @user.nickname = ''
        @user.valid?
        expect(@user.errors.full_messages).to include("ニックネームを入力して下さい")
      end
      it 'emailが空では登録できない' do
        @user.email = ''
        @user.valid?
        expect(@user.errors.full_messages).to include("Eメールを入力して下さい")
      end
      ...
      it 'emailは@を含まないと登録できない' do
        @user.email = 'test.com'
        @user.valid?
        expect(@user.errors.full_messages).to include('Eメールは@を含まみます')
      end
      ...
      it 'passwordとpassword_confirmationが不一致では登録できない' do
        ...
        @user.valid?
        expect(@user.errors.full_messages).to include("パスワードとパスワード（確認用）の入力が一致しません")
      end
